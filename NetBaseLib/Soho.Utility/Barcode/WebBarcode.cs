...
namespace Common.Utility.Barcode
{
    public class WebBarcode
    {
        /// <summary>
        /// 生成39码 可表示数字、字母等信息的条码
        /// </summary>
        /// <param name="code">条码内容</param>
        /// <param name="width">宽度</param>
        /// <param name="height">高度</param>
        /// <returns></returns>
        public static string Generate39Code(string code, int width, int height)
        {
        ...
        /// <summary>
        /// 生成EAN13码 欧洲物品编码，共计13位
        /// </summary>
        /// <param name="code">条码内容</param>
        /// <param name="width">宽度</param>
        /// <param name="height">高度</param>
        /// <returns></returns>
        public static string GenerateEAN13Code(string code, int width, int height)
        {
            //输入的校验码
            int checkcode_input = -1;
            if (!Regex.IsMatch(code, @"^\d{12}$"))
            {
                ...
                {
                    checkcode_input = int.Parse(code[12].ToString());
            ...
            for (int i = 0; i < 12; i++)
            {
                if (i % 2 == 0)
                {
                    sum_odd += int.Parse(code[i].ToString());
                }
                else
                {
                    sum_even += int.Parse(code[i].ToString());
                }
            }
            ...
            string type = ean13type(code[0]);
            for (int i = 1; i < 7; i++)
            {
                result_bin += ean13(code[i], type[i - 1]);
            }
            result_bin += "01010";
            for (int i = 7; i < 13; i++)
            {
                result_bin += ean13(code[i], 'C');
            }
            ...
            result_html += "<div style=\"float:left;color:#000000;width:" + (width * 9) + "px;text-align:center;\">" + code[0] + "</div>";
            ...
            for (int i = 1; i < 7; i++)
            {
                result_html += "<div style=\"float:left;width:" + (width * 7) + "px;color:#000000;text-align:center;\">" + code[i] + "</div>";
            }
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#FFFFFF;\"></div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#000000;\"></div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#FFFFFF;\"></div>";
